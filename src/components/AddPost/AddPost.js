...
const AddPost = (props) => {
    ...
    return (
        <Paper className='AddPost' elevation={3}>
            ...
            <input type="text" className="message" placeholder="Write Message" value={realPost.text} onChange = {(e) => setRealPost({...realPost, text: e.target.value})}/>
            <img src={selector} className='hidden-img'/>
            <GifDropDown selectGif={selectGif}/>
            <Button variant="contained" onClick={postClick} style={{maxWidth: '50px', maxHeight: '30px', minWidth: '30px', minHeight: '30px', fontSize: '10px'}} className="post-btn">Post</Button>
        </Paper>
    )
}
